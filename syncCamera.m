function [cams] = syncCamera(parameter_file)
...
cd(exp_path)
x = input('type 1 for whisker camera, type 2 for face camera')
switch x
    case 1
...
csv_times = csv_times + ((strobe_start - double(full_timestamps(1))) / open_ephys_TTL.Header.sample_rate);
csv_frames = floor(csv_times * open_ephys_TTL.Header.sample_rate);
% save the timestamps and frame times
cams.strobe_start_frame = strobe_start - full_timestamps(1);
cams.csv_start = csv_start;
cams.aligned_times = csv_times;
